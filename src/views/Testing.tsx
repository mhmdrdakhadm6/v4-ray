...
export function Testing() {
  ...
  const hasCurrent = progress.currentServer != null

  return (
    <div className="flex flex-col items-center gap-8 text-center">
      <h1 className="text-2xl font-bold text-orange-50 sm:text-3xl">Testing from your connection…</h1>
      ...
      {hasCurrent ? (
        <GlassCard className="w-full max-w-md px-6 py-4">
          <p className="text-xs uppercase tracking-widest text-orange-200/50">Pinging</p>
          <p className="mt-2 text-lg font-semibold text-orange-50">
            {progress.currentServer ?? 'resolving'} —{' '}
            <span className="text-ember-300">{protocolBadge(progress.currentProtocol ?? '')}</span>
            {progress.currentLatency != null && (
              <span className="ml-2 text-orange-200/60">{progress.currentLatency} ms</span>
            )}
          </p>
        </GlassCard>
      ) : (
